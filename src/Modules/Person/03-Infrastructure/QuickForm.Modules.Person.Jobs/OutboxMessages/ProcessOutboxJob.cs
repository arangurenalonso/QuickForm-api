    ...
    ) : IJob
{
    ...
    {
        _logger.LogInformation("{Module} Beginning to process outbox messages", Schemas.Person);
        ...
        _logger.LogInformation("{Module} - Completed processing outbox messages", Schemas.Person);
    }
          ...
          IDbTransaction transaction)
    {
        string sql = $"""                                    
                      SELECT TOP ({_outboxOptions.Value.BatchSize}) 
                        id, 
                        content
                      FROM [{Schemas.Person}].[outbox_messages] WITH (ROWLOCK, UPDLOCK)
                      WHERE Status = 0
                ...
                List<ResultError> listResultError)
    {
        ...
        const string sql = $"""
                                UPDATE {Schemas.Person}.[outbox_messages]
                                SET 
                                    Status = @Status,
